...
class Sniffer:
    def __init__(self, input_iface=None, output_iface=None, device=None, queue_num=0, iptables_backup=None):
        ...
        if self.device:
            ...
            if self.device.ip:
                ...
                # os.system("iptables -I OUTPUT -o {} -d {} -j NFQUEUE --queue-num {}".format(
                #     self.input_iface,
                #     self.device.ip,
                #     self.QUEUE_NUM
                # ))
                # logger.info("Forwarding OUTPUT packtes sent to %s to NFQUEUE (queue_num = %d).", self.device.ip, self.QUEUE_NUM)
        else:
    ...
    def cleanup(self):
        ...
        if self.device:
            ...
            if self.device.ip:
                ...
                # os.system("iptables -D OUTPUT -o {} -d {} -j NFQUEUE --queue-num {}".format(
                #     self.input_iface,
                #     self.device.ip,
                #     self.QUEUE_NUM
                # ))
                logger.info("Removing IP-related iptables rules (IP=%s, queue_num=%d).", self.device.ip, self.QUEUE_NUM)
        # logger.info("iptables cleaned up! (using backup file: %s)", self.iptables_backup_fp)
