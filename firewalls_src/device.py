...
class Device:
    ...
    """
    def __init__(self, mac="", ip="", name="", product="", feature="", pattern_enabled=False, thresh=0, nslookup_loop=1):
        self.mac = mac
        self.ip = ip
        self.name = name
    ...
    def _set_allowlist(self):
        ...
        if self.feature == 'domain':
            hostname_rule_fp = f'{constants.ALLOWLIST_DIR}/{self.product}/hostname/1.csv'
    ...
    def enable_pattern_matching(self, allowlist=None, inplace=False):
        fp = f'./data/allowlists/{self.product}/patterns/{self.product}-pattern-map.json'
